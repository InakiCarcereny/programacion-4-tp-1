...
import { alumnos, type Alumno } from "../models/db.ts";
...
export function obtenerMejorAlumno(alumnos: Alumno[]): Alumno | undefined {
  ...
  return alumnos.reduce((mejor, alumno) => {
...
export function calcularPromedioPorCiudad(
  ...
  return (
    alumnosDeCiudad.reduce((suma, alumno) => suma + alumno.nota, 0) /
    alumnosDeCiudad.length
  );
}
...
console.log(calcularTotal(alumnos, (alumno) => alumno.edad));
