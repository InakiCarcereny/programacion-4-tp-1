...
import { Vehiculo, Auto, Moto, Camion } from "../parte 3/ej14-vehiculos.ts";
...
describe("Ejercicio 14 - Auto", () => {
  it("acelerar() menciona 'auto'", () => {
    ...
    expect(String(logSpy.mock.calls[0]?.join(" ")).toLowerCase()).toContain(
      "auto",
    );
  });
});
...
describe("Ejercicio 14 - polimorfismo", () => {
  it("cada vehículo del arreglo ejecuta SU PROPIO acelerar(), no el de Vehiculo", () => {
    ...
    const mensajes = logSpy.mock.calls.map((call) =>
      String(call.join(" ")).toLowerCase(),
    );
